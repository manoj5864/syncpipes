...
let excelMysqlMapper = function(excelService, mysqlService, excelDataFactory, dataFactory){
    "use strict";

    var control = function($scope, excelService, excelDataFactory, dataFactory) {
        $scope.init = function() {
            var tables = dataFactory.getTables();
            for(var i=0; i<tables.length; i++)
                getColumns(dataFactory.getDatabase(), tables[i]);

            $scope.excelSheets = excelDataFactory.getExcelSheets();
            $scope.tables = dataFactory.getTables();

            $scope.objectMapper = excelService.getObjectMapper(excelDataFactory, dataFactory);
        }
    }
    ...
    var linker = function (scope, element, attrs) {
        ...
        scope.getExcelColumn = function(sheet) {
            return excelService.getColumnsInExcelSheet(sheet, excelDataFactory.getData());
        };
...
excelMysqlMapper.$inject = ['excelService', 'mysqlService', 'excelDataFactory', 'dataFactory'];
